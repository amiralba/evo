...
- **Surface the tradeoff/evidence and make every decision defensible — never auto-decide.** *(adopted v0.5, from the GPT brainstorm through-line)* The system narrows, ranks, previews impact and records reasons; the human chooses. Applies uniformly: publish gate (errors pass only with a written justification), Onarım (candidates ranked, planner picks), rule changes (impact preview first), analytics (evidence chain, no causality claims).
- **One concept per job.** If an existing concept covers a need, extend it instead of adding a parallel one — e.g. "campaign" was deleted as a concept once TaskTemplate gained `target` + `valid_until` (v0.5).
...
> **Build note (spec 009, 2026-07-17):** implemented as `visit_realization` — a **separate table**
> 1:1 with `planned_visit` (not a merged "Visit" row as sketched above), holding check-in/out time,
> actual minutes, and an outcome reason; `planned_visit.status` (Done/Missed/Skipped) is the outcome
> field, a smaller 3-value set than the 5-code taxonomy above (`REFUSED`/`NO_GPS_CONFIRM`/`PARTIAL`
> collapse into `Missed`/`Skipped` + `VisitOutcomeReason` for M3). GPS is not a per-visit geofence
> point but a **continuous `merchandiser_location_ping` stream** (pulled forward from the §6.2/M4
> live-location layer per user decision) — a visit's check-in point is derived as the nearest ping.
> See `docs/DECISIONS.md` (2026-07-17) for the full rationale; revisit the 5-code taxonomy if M4
> analytics needs the finer distinction.
...
| `target` *(v0.5)* | Optional store filter `{format?, chain?}` — template applies only to matching stores' task lists. Empty = all stores |
| `valid_until` *(v0.5)* | Optional expiry date — template drops from all task lists automatically when passed; surfaces as 🔵 info while active |
| `active` | Templates are versioned, never deleted |

> **v0.5 decision — campaigns removed as a concept.** A "campaign" was exactly a TaskTemplate with `recurrence=ONCE` + `target` + `valid_until`, so the separate campaign entity/modal/tabs were deleted. One concept, one modal, one rule ladder; deadline tracking (attach to first visit before `valid_until`, OVERDUE escalation) is execution-side behavior of any `ONCE` task (V11 unchanged).
...
> **Build note (spec 009, 2026-07-17):** the schema landed as designed, but with no live
> field-agent write path — the mobile app is deferred, so M3 notes are **seeder-produced only**;
> `author_id` is nullable and mostly null in seeded data. Revisit once mobile is revived.
...
> **Build note (spec 009, 2026-07-17):** M3 landed the `notification` table and a real write
> path — `INotificationDispatcher` (mock impl) writes one row per assigned merchandiser when a route
> publishes — but with **no batching window** (one row per publish, not collapsed per-day) and **no
> real delivery** (no FCM push; `read_at` is seeder-simulated). Batching + real push are mobile-revival
> concerns.
...
| V12 *(v0.5)* | Visit overlap (same person/day) | Error |
| V13 *(v0.5)* | Travel-time infeasible (gap between consecutive visits < travel estimate/override) | Error |
...
> **Build note (spec 009, 2026-07-17):** there is no real external tracking feed to reuse (mobile is
> deferred), so this table is effectively realized early as `merchandiser_location_ping` (plain
> `lat`/`lng` doubles, not `geography(Point)` — no spatial query needs it yet) — pulled forward from
> M4 into M3 per user decision so the seeder could produce realistic check-in history now. The
> read-only live-map layer described here is **not** built — only the data + a paged read API
> (`GET /merchandisers/{id}/location-history`) exist. Build the map layer against this table when M4
> starts rather than re-designing the schema.
...
### 7.3b Absence — ✨ Onarım decision workbench *(updated v0.5; was "manual, by design")*
...
1. ~~**Travel time between stops**~~ — **RESOLVED v0.5:** layered model **Elle > GPS gerçekleşen > Tahmin** (closest rule wins). Prototype estimates from coordinates; real build: OSRM (OSM Türkiye) for estimates, GPS checkout→checkin averages above it, per-pair manual override (edited by clicking the 🚗 connector) always wins and is audited. Direction (A→B ≠ B→A) and time-of-day belong to the estimate layer. Infeasible travel = V13 error.
2. ~~**Over-450 policy**~~ — **RESOLVED v0.5:** warn-with-logged-override generalized into the publish gate: all 🔴 errors publishable only with written justification + objective, recorded to the Decision Journal. Nothing hard-blocks.
...
## 11. v0.5 Prototype Iteration (Tem 2026)

Built after the GPT/Gemini design probe (`gpt-brainstorm.md`) and its triage (`v0.5-change-list.md`). Prototype: `evo-planner-prototype-v0.5.html` (v0.4 preserved in `older ver/`). Verified by an automated jsdom test suite (21 scenario tests + cascade regression tests).

### 11.1 Fixes (v0.4.1)

- **Cascade bug:** Baz edits (e.g. frequency change) wiped all patches via `weekData={}`; future weeks were deleted rather than reprojected. Now: future weeks reproject from Baz **preserving patches** (Override > Base — base edits never clobber a live override), only the rebuilt store's own visits regenerate, past weeks untouched.
- **Cache desync:** flows that reassigned `visits` (deactivate, move-to-pool, route delete, revertPatch) never wrote back to `weekData[currentWeek]`, so removed visits resurrected on week navigation. Closed.

### 11.2 New in v0.5 (per feature, with the decision behind it)

| Feature | Decision / rationale |
|---|---|
| **Severity model + Sorun Merkezi** (V12–V16, §3.2 note) | Compiler-style err/warn/info; problems come to the planner (jump-to-visit) instead of being hunted across 400 stores. Scope-based placement prevents "Christmas lights". |
| **Override-with-reason publish gate + Decision Journal** | Our synthesis of GPT's Q5 (hard block-list) + Q13 (decision accountability): errors never block, but demand justification + business objective, journaled with author. "The system recommended it" is never the answer on record. |
| **✨ Onarım = manual decision workbench** (§7.3b) | No auto-generated repair plans (scale/trust decision — see 7.3b). System narrows + ranks candidates; planner decides per visit; partial decisions allowed. |
| **⚡ Otomatik düzelt** | Same-person/same-day time-shift only (opens overlap/travel gaps). Mechanical, deterministic — never picks an agent or day. |
| **Travel layer** (Open Q#1 resolved) | Elle > GPS > Tahmin cascade; per-pair override via 🚗 connector; feeds V13 + auto-fix uniformly. |
| **Event-aware patch windows** | Patch scope options offer events the system already knows ("mağaza açılana dek", "X'in izni boyunca") instead of raw dates; windowed patches auto-revert. |
| **"Kalıcı yap" impact confirm** | Writing a patch to Baz first shows how many future visits it touches. |
| **Rule Inspector** (store Görevler tab) | Dev-tools-style provenance per duration: şablon → tip → rut → mağaza chain with arithmetic — answers "why is this store different?". |
| **Rule-change Impact Preview** | Any scoped duration rule (drag-resize scopes, ✎ modal) previews ripple first: stores · visits/week · Δdk/week · days pushed over quota. |
| **Value layer (Q8)** | Purpose-per-visit, inline value strip (▲%12 ★★★★), Planlama Kanıtı panel with evidence chain (plan → execution → shelf → sales) and explicit no-causality-claim framing — extends §8's anti-mobbing ethic to the planning surface. |
| **Campaigns deleted as a concept** (§2.8) | TaskTemplate gained `target` + `valid_until`; kampanya = süreli hedefli görev. Toolbar 📣, admin tab, table tab, inbox section all removed. |
| **Gelen kutusu = two tabs** | 💬 Saha (messages/requests/overdue) + ⚠ Sorunlar (the severity list — status-bar counters and day-header badges open here). One home for "things needing attention"; the separate Sorun Merkezi modal was removed. |
| **Keyboard & navigation** | `/` or `Cmd/Ctrl+K` focus search; Enter selects first result and scrolls-to + flashes the visit block; help guide updated; help icon plain black "?". |

### 11.3 Entity/model additions implied for the real build

- `travel_override(store_a, store_b, minutes, author, at)` — manual layer of the travel cascade (consider direction + time-of-day columns).
- `decision_journal(kind, desc, reason, objective, author, at, errors[])` — publish-with-errors, repair decisions, permanents; distinct from `route_change_log` (this is the *why*, that is the *what*).
- `task_template.target (jsonb)`, `task_template.valid_until (date)` — replaces campaign table.
- `person.leave` (from HR/mobile sync) and `store.closed_until` — drive V14 + Onarım disruptions + event-aware patch windows.
- `store.perf` (compliance %, actual minutes, shelf scores, sales trend) — fed back from execution pipeline; mock in prototype.
- Visit `purpose` (enum/text) — visits carry intent, not just duration.

### 11.4 Deliberately not built (still designed-only)

Setup Mode / bulk cold-start (Q9 two-engine model), publish communication loop (delivery/read/ack/"Anladım–Sorunum var"), repair solver / Coverage Decision (Q10), per-user drafts + OCC (Q11), policy governance (Q13), planned-vs-actual on past weeks. Tech stack proposal for the real build: `EVO-Teknoloji-Yigini.pdf`.
