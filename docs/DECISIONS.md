...
## 2026-07-20 — `cleanup/c1-safe-deletions` merged into `main`; `main` is now the working branch
- **Decision:** The audit branch `cleanup/c1-safe-deletions` (sessions C1–C5: dead-tree deletion, security/correctness hardening, doc sync) was merged into `main`. `main` is the active working branch going forward. `prototype-parity-rebuild` remains as a backup branch only, not active.
- **Why:** The audit pass (see the two 2026-07-19 entries below) completed and verified (171/171 backend, panel green) on the audit branch; merging closes it out rather than leaving long-lived divergence from `main`.
- **Consequences:** Any doc or spec referencing `cleanup/c1-safe-deletions` or `prototype-parity-rebuild` as "the active branch" is stale as of this date — corrected in this sync pass.

## 2026-07-20 — Store/route/schedule 4-layer model shipped: store status endpoint, remove-stop-to-pool endpoint, weekday-mask scheduling
- **Decision:** Landed the four-layer store/route/schedule editing model, all draft-until-publish, no new DB tables/columns (both `RouteStop.WeekdayMask` and `Store.Active` already existed in schema): **L1** store activate/deactivate — new `PATCH /stores/{id}/status` body `{"active": bool}`; deactivating keeps `RouteStop` membership open but `PlanGenerationService` now skips stops on inactive stores and regenerates every route holding the store. **L2** route deactivate — already existed (`PATCH /routes/{id}` status), unchanged. **L3** remove-from-route → pool — new `DELETE /routes/{id}/stops/{stopId}`, 204, soft-closes the `RouteStop` (`EffectiveTo = today`, no-delete rule) + regenerates + logs `RouteChangeEvent.StopRemoved`. **L4** schedule days — `PATCH /routes/{id}/stops/{stopId}` gained `weekdayMask` (int16); `frequency=Weekly` + a mask restricts a stop to chosen weekdays (bit0=Mon…bit4=Fri, e.g. Mon/Wed/Fri = 21).
- **Why:** Closes the gap between the design's four independent layers of "is this store/route/stop/day in the plan" and what the API exposed — previously only route-level deactivate and unconstrained weekly frequency existed; stores could only be dropped from a route by editing stops one at a time with no soft-close semantics, and there was no way to pin a stop to specific weekdays.
- **Alternatives rejected:** Hard-deleting the `RouteStop` on store deactivation — rejected, violates the no-delete rule and would lose history attached to the stop. Adding new schema columns for weekday selection — rejected, `WeekdayMask` already existed unused; reusing it avoids an unnecessary migration.
- **Consequences:** `StoreGeoDto` gained an `Active` field. `docs/API.md`/`docs/DATABASE.md` updated in this sync. Backend 171/171, panel Vitest 40/40, `tsc` clean, lint 0 errors (the standing `public/evo-prototype/engine.js` eslint parse error is now fixed via eslint-ignoring that vendored path, same treatment as `src/api/generated`).

## 2026-07-19 — Audit P0/P3/P2 hardening pass: PlanningClock, security baseline, correctness bugs, first scale steps
- **Decision:** Executed the audit backlog's P0 (trust & safety), P3 (correctness) and the contained half of P2 (scale) in one session on `cleanup/c1-safe-deletions`. Key choices: (1) planning "today" is a **Europe/Istanbul calendar date** via `PlanningClock(TimeProvider)` — tests pin `FakeTimeProvider` to Wed 2026-07-15, ending the weekend-failure era (suite now 171/171); (2) XSS defense is **boundary escaping** in `backendBridge.esc` (+`unesc` on the two round-trip fields) + CSP, NOT in-engine escaping — that moves with D2b; (3) analytics output cache is **shared across authenticated Supervisors** (`excludeDefaultPolicy`) because the 2-role all-regions model makes responses user-independent; (4) EF retry-on-failure deliberately NOT enabled — it conflicts with the new explicit transactions on Reassign/Publish/status transitions; (5) `--wipe`-style deferrals recorded per item in `docs/audit/TODO-from-audit.md` (batch plan endpoint, 202 regen queue, region-required analytics, AuditWriter semantics, rowversion, `route_day_stats`).
- **Why:** Audit §G priority order; each P3 item was a verified real bug (chain-targeted templates matching every chain, absence errors bypassing the publish gate, 450/42 constants desynced from settings, add-patch visits silently collapsing on the Guid.Empty sentinel, partial-commit reassignment).
- **Consequences:** Schema migration `AuditP3RestrictCascadesFilteredVisitKeyIndexes` (all FKs Restrict — "no delete" is now schema-enforced; filtered visit-key unique index; two new planned_visit indexes). Access tokens 15 min. Auth endpoints rate-limited per IP. CI needs its first real push/PR to prove the new SQL service containers. OnarimApplyTests now select the disruption containing their own visit (the pinned clock means stale test absences never age out — that selection bug was also the historic CrossReassignVisit flake).
