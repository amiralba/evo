...
```
React panel (planner SPA)      React Native app (agents)
        │  generated TS client         │  generated TS client + sync queue
        ▼                              ▼
...
| Mobile | Read-only week view + notes; offline-first; GPS check-in; photo upload | React Native (Expo), WatermelonDB, sync queue |
