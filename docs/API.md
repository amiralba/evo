...
| Stores/map | **LANDED (spec 006).** `GET /stores/geo?province=&district=&onRoute=` — any authenticated user; bulk `StoreGeoDto[]` (lat/lng, chain/category/format, active-route id+code if any, 6-month revenue), required `province`, capped at 5000. Feeds the planner map's store layer. Deferred: bbox/polygon spatial query (client-side turf point-in-polygon lasso instead), `GET /stores/{id}/summary`, `GET /stores/{id}/task-plan`. | 006 |
