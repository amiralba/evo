...
- Milestone: M0 — Platform foundation (in progress). Spec 001-solution-scaffold COMPLETE (all 18 tasks). Spec 002-auth-roles COMPLETE (all 33 tasks). Spec 003-error-audit COMPLETE (all 22 tasks, 4 phases committed to main) — this was the LAST spec in the original platform-spec order.
- Active feature: none in progress — next step is `/plan` for spec 004-store-sync (EVO sales sync ingestion), the last platform spec before feature modules begin per ROADMAP.
- Last session summary: Built the full 003-error-audit: domain exception taxonomy (EvoException → NotFoundException 404, ConflictException 409, EvoValidationException 422) + `AddProblemDetails(CustomizeProblemDetails)` + `IExceptionHandler` + custom InvalidModelStateResponseFactory, all on ASP.NET Core's built-in ProblemDetails pipeline (no hand-rolled middleware). Mid-implementation scope amendment requested by the human after the Phase 1 checkpoint: error responses now carry three tiers of text — `code` (machine key), `title`/`detail` (English, dev-facing), and new `userTitle`/`userMessage` (Turkish, user-facing) resolved from an in-code catalog (`Evo.Domain.Errors.UserErrorMessages`), explicitly NOT a DB table ("no db is bad, for every error we connect to db") — this also simplified Phase 4 (panel just displays backend-provided `userMessage`, no client-side Turkish map). Generic append-only `audit_log` table + `IAuditWriter` (collapsing the design doc's route_change_log + admin_audit_log, which lack owning entities yet); supervisor-only paged `GET /audit-log`; AuthController/UsersController retrofitted onto the unified shape and now write real audit rows. docs/API.md, docs/AUTH.md, docs/DATABASE.md, docs/DECISIONS.md updated. Verified: 28/28 backend xUnit, 7/7 panel Vitest, 2/2 Playwright, full manual browser walkthrough (Turkish error message sourced live from backend on wrong credentials, successful login, dashboard renders).
