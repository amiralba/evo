...
- Milestone: M0 — Platform foundation (in progress). Spec 001-solution-scaffold COMPLETE (all 18 tasks). Spec 002-auth-roles COMPLETE (all 33 tasks, 4 phases committed to main).
- Active feature: none in progress — next step is `/plan` for spec 003-error-audit (shared ProblemDetails shape, RouteChangeLog + admin_audit_log). Note: spec 002's auth/users endpoints currently use ASP.NET Core's built-in `Problem(...)` as an interim error shape (flagged in docs/AUTH.md and docs/API.md) — 003 must unify/refine this, not leave it as a one-off. After 003: 004-store-sync (EVO sales sync ingestion) — then feature modules per ROADMAP.
- Last session summary: Built the full 002-auth-roles: ASP.NET Identity on EvoDbContext (IdentityDbContext<ApplicationUser, IdentityRole<Guid>, Guid> + RefreshTokens table, AddIdentity migration), Evo.Seeder IdentitySeederModule (roles + bootstrap admin + demo Field agents); JwtTokenService + RefreshTokenService (rotating refresh cookie) + AuthController (login/refresh/logout) with an AddEvoAuthentication Entra extension seam; hardened refresh-token rotation per automated security review (RowVersion optimistic concurrency for TOCTOU, reuse-detection revokes all sessions, startup config validation rejects weak/dev signing keys outside Development); GET /me, change-password, supervisor-only UsersController (create/list/get/update/activate/deactivate, no delete — a 403-vs-400 status fix was applied per human request); docs/AUTH.md written, docs/API.md + docs/DECISIONS.md updated; panel auth integration (react-router-dom, in-memory session store, AuthContext, 401-interceptor-with-retry, Login/Dashboard pages, ProtectedRoute). Verified: 15/15 backend xUnit, 5/5 panel Vitest, 2/2 Playwright, full manual browser walkthrough (login, wrong-password error, logout, reload-stays-logged-out).
